function createUnityInstance(e, r, t) {
    function n(e, t) {
        if( !n.aborted&&r.showBanner)return"error"==t&&(n.aborted= !0),
        r.showBanner(e, t);

        switch(t) {
            case"error": console.error(e);
            break;
            case"warning": console.warn(e);
            break;
    ...
    function o(e) {
        ...
        t=r?r.toString(): e.message||e.reason||"", n=r&&r.stack?r.stack.toString():"";
    ...
    function i(e, r) {
        if("symbolsUrl" !=e) {
            ...
            for(var e in c.downloadProgress) {
                ...
                s++,
                n.lengthComputable?(o+=n.loaded, a+=n.total, i++): n.finished||d++
            }
    ...
    function d(e) {
        return new Promise(function(r, t) {
                i(e); var o=c.companyName&&c.productName?new c.XMLHttpRequest({
                    companyName:c.companyName, productName:c.productName, cacheControl:c.cacheControl(c[e])

                }):new XMLHttpRequest; o.open("GET", c[e]), o.responseType="arraybuffer", o.addEventListener("progress", function(r) {
                    i(e, r)
                    ...
                    var t="Failed to download file " +c[e]; "file:"==location.protocol?n(t+". Loading web pages via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host Unity content, or use the Unity Build and Run option.", "error"):console.error(t)
                }), o.send()
        })
}
...
function l() {
    return new Promise(function(e, r) {
            var t=document.createElement("script"); t.src=c.frameworkUrl, t.onload=function() {
                if("undefined"==typeof unityFramework|| !unityFramework) {
                    var r=[["br", "br"], ["gz", "gzip"]]; for(var o in r) {
                        var a=r[o]; if(c.frameworkUrl.endsWith ("." +a[0])) {
                            var s="Unable to parse " +c.frameworkUrl+"!"; if("file:"==location.protocol)return void n(s+" Loading pre-compressed (brotli or gzip) content via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host compressed Unity content, or use the Unity Build and Run option.", "error"); if(s+=' This can happen if build compression was enabled but web server hosting the content was misconfigured to not serve the file with HTTP Response Header "Content-Encoding: ' +a[1]+'" present. Check browser Console and Devtools Network tab to debug.', "br"==a[0]&&"http:"==location.protocol) {
                                var i=["localhost", "127.0.0.1"].indexOf(location.hostname) !=-1?"":"Migrate your server to use HTTPS."; s=/Firefox/.test(navigator.userAgent)?"Unable to parse " +c.frameworkUrl+'!<br>If using custom web server, verify that web server is sending .br files with HTTP Response Header "Content-Encoding: br". Brotli compression may not be supported in Firefox over HTTP connections. ' +i+' See <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=1670675">https://bugzilla.mozilla.org/show_bug.cgi?id=1670675</a> for more information.':"Unable to parse " +c.frameworkUrl+'!<br>If using custom web server, verify that web server is sending .br files with HTTP Response Header "Content-Encoding: br". Brotli compression may not be supported over HTTP connections. Migrate your server to use HTTPS.'
                            }
                ...
                var d=unityFramework; unityFramework=null, t.onload=null, e(d)
            }
...
function u() {
    ...
    c.preRun.push(function() {
            c.addRunDependency("dataUrl"), e.then(function(e) {
                    var r=new DataView(e.buffer, e.byteOffset, e.byteLength), t=0, n="UnityWebData1.0\0"; if( !String.fromCharCode.apply(null, e.subarray(t, t+n.length))==n)throw"unknown data format"; t+=n.length; var o=r.getUint32(t, !0); for(t+=4; t<o; ) {
                        var a=r.getUint32(t, !0); t+=4; var s=r.getUint32(t, !0); t+=4; var i=r.getUint32(t, !0); t+=4; var d=String.fromCharCode.apply(null, e.subarray(t, t+i)); t+=i; for(var l=0, u=d.indexOf("/", l)+1; u>0; l=u, u=d.indexOf("/", l)+1)c.FS_createPath(d.substring(0, l), d.substring(l, u-1), !0, !0); c.FS_createDataFile(d, null, e.subarray(a, a+s), !0, !0, !0)
                    }
...
return c.SystemInfo=function() {
    ...
    for(var r, t, n, o, a, s, i=navigator.userAgent+" ", d=[["Firefox", "Firefox"], ["OPR", "Opera"], ["Edg", "Edge"], ["SamsungBrowser", "Samsung Browser"], ["Trident", "Internet Explorer"], ["MSIE", "Internet Explorer"], ["Chrome", "Chrome"], ["CriOS", "Chrome on iOS Safari"], ["FxiOS", "Firefox on iOS Safari"], ["Safari", "Safari"]], l=0; l<d.length; ++l)if(t=e(d[l][0]+"[/ ](.*?)[ \\)]", i, 1)) {
        r=d[l][1];
        break
    }
    ...
    a&&(gl=a.getContext("webgl2"), glVersion=gl?2:0, gl||(gl=a&&a.getContext("webgl"))&&(glVersion=1), gl&&(s=gl.getExtension("WEBGL_debug_renderer_info")&&gl.getParameter(37446)||gl.getParameter(7937)));
...
function a(r) {
    ...
    ,
    r&&(this.cache.control=r.cacheControl, this.cache.company=r.companyName, this.cache.product=r.productName),
    this.xhr=new XMLHttpRequest(r),
    this.xhr.addEventListener("load", function() {
            var r=this.xhr, t=this.cache; t.enabled&& !t.revalidated&&(304==r.status?(t.result.revalidated=t.result.accessed, t.revalidated= !0, l.execute(i.name, "put", [t.result]), e("'" +t.result.url+"' successfully revalidated and served from the indexedDB cache")):200==r.status?(t.result=o(t.result.url, t.company, t.product, t.result.accessed, r), t.revalidated= !0, l.execute(i.name, "put", [t.result], function(r) {
...
a.prototype.send=function(r) {
    ...
    o.enabled?void l.execute(i.name, "get", [o.result.url], function(r) {
            if( !r||r.version !=i.version)return void n.send.apply(n, a); if(o.result=r, o.result.accessed=Date.now(), "immutable"==o.control)o.revalidated= !0, l.execute(i.name, "put", [o.result]), n.dispatchEvent(new Event("load")), e("'" +o.result.url+"' served from the indexedDB cache without revalidation"); else if(t(o.result.url)&&(o.result.responseHeaders["Last-Modified"]||o.result.responseHeaders.ETag)) {
                var s=new XMLHttpRequest; s.open("HEAD", o.result.url), s.onload=function() {
...
a.prototype.open=function(e, t, n, a, s) {
    ...
    this.cache.enabled=["must-revalidate",
    "immutable"].indexOf(this.cache.control) !=-1&&"GET"==e&&this.cache.result.url.match("^https?://")&&("undefined"==typeof n||n)&&"undefined"==typeof a&&"undefined"==typeof s,
